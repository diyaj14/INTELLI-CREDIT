...
class FileClassifier:
    ...
    BANK_KEYWORDS    = ["bank", "statement", "account", "passbook",
                        "ledger", "current", "saving", "ca", "ods"]

    def classify(self, filepath: str) -> str:
        """
        Returns one of: 'annual_report' | 'gstr3b' | 'gstr2a' | 'bank_statement' | 'unknown'
        """
    ...
    if annual_reports:
        ...
        for fp in annual_reports:
            try:
                ...
                logger.info(f"Ingested {fp}: {ingest_result['page_count']} pages, "
                            f"confidence={ingest_result['confidence']:.2f}")
        ...
        if combined_text:
            ...
            logger.info(f"Financial extraction done. confidence={fin_confidence:.2f}")
    else:
        logger.warning("No annual report PDFs provided — using default financial values")
