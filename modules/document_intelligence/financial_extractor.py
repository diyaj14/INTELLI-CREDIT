...
class FinancialExtractor:
    ...
    def extract_field_llm(self, field_name: str) -> tuple[Optional[float], str]:
        """
        Uses LLMs to extract a field that regex couldn't find.
        Fallback sequence: Gemini -> Groq -> OpenRouter.
        """
        field_descriptions = {
        ...
        # Search for pages mentioning Balance Sheet or P&L to prioritize extraction
        best_pages = []
        for pg, type_str in self.page_types.items():
            if type_str in ["Profit & Loss", "Balance Sheet"]:
                best_pages.append(pg)
        
        # If no specific types found, take first 6. Otherwise take prioritised pages + some buffer
        if not best_pages:
            best_pages = sorted(self.raw_text.keys())[:10]
        else:
            # Take the prioritized pages plus some context pages
            best_pages = sorted(list(set(best_pages)))[:15]

        sample_text = "\n\n---PAGE BREAK---\n\n".join(
            [f"--- Page {p} ---\n{self.raw_text.get(p, '')}" for p in best_pages]
        )
...
- If value is raw rupees (not in lakhs/crores), convert to Crores (divide by 10000000)
- For ratios: return the number as-is
...
"""
        ...
        # 1. Try Gemini first
        gemini_model = self._load_gemini()
        if gemini_model:
            try:
                response = gemini_model.generate_content(prompt)
                val = self._parse_llm_response(response.text, field_name, "Gemini Flash")
                if val is not None:
                    return val, "Extracted via Gemini Flash"
            except Exception as e:
        ...
        if groq_api_key:
            try:
                ...
                if resp.status_code == 200:
                    ...
                    val = self._parse_llm_response(text_resp, field_name, "Groq Llama")
                    if val is not None:
                        return val, "Extracted via Groq (Llama 70B)"
            except Exception as e:
        ...
        if or_api_key:
            try:
                ...
                if resp.status_code == 200:
                    text_resp = resp.json()["choices"][0]["message"]["content"]
                    val = self._parse_llm_response(text_resp, field_name, "OpenRouter DeepSeek")
                    if val is not None:
                        return val, "Extracted via OpenRouter (DeepSeek)"
            except Exception as e:
                logger.warning(f"OpenRouter extraction failed: {e}")

        return None, ""

    def extract_unstructured_risks(self, doc_type: str = "annual_report") -> List[str]:
        """
        Extracts high-level risks, financial commitments, and red flags from 
        unstructured text (Legal notices, Sanction letters, etc.)
        """
        model = self._load_gemini()
        if not model:
            return ["Gemini API key missing - risk extraction skipped."]

        # Combine text from all pages
        full_text = "\n".join(list(self.raw_text.values()))[:15000] # Cap for safety
        
        prompt = f"""You are a Credit Risk Analyst. Analyze this {doc_type} and extract key risks.
        
        Extract:
        1. Financial Commitments (Loans mentioned, interest rates, repayment dates)
        2. Legal Risks (Court cases, summons, penalties)
        3. Operational Risks (Factory issues, labor strikes, capacity problems)
        
        Return a JSON list of strings, each being a short, impactful risk statement.
        If no major risks found, return [].
    ...
    def _parse_llm_response(self, raw: str, field_name: str, provider: str) -> Optional[float]:
        raw = raw.strip()
        ...
        try:
            ...
            structured = self._make_citation(round(val, 2), field_name, None, f"LLM extraction via {provider}", "llm")
            self._citations_structured[field_name] = structured
            return round(val, 2)
        except ValueError:
            return None
